...
<body class="is-preload">
	...
	ArrayList<ReservationJoinVO> list = (ArrayList<ReservationJoinVO>) session.getAttribute("Reserve");
...
현재 주문한 예약이 없습니다😓<br>
<a href="Main.jsp">주문하러 가기</a><br><br><br>
<button type="button" class="navyBtn" onClick="location.href='Main.jsp'">주문하러 가기</button>
<%}else{ %>

<% 
for(int i=0;i<list.size();i++){%>
<input type="hidden" id="p_Time" value="<%=list.get(i).getP_time()%>" >
<input type="hidden" id="r_Time" value="<%=list.get(i).getR_time()%>" >
<input type="hidden" id="c_Time" value="<%=sdao.store_Select_Storeid((list.get(i).getStore_Id()))%>" >
<%}%>
<br>
	<br>
<h4>주문 진행 현황</h4>
...
%>
        <tr>
          ...
          <td>
	          <span><%=rnum%> <%=list.get(i).getR_number() %></span><br>
	          <span><%=today %> <%=list.get(i).getR_time()%></span><br>
	          <span><%=today %> <%=list.get(i).getP_time()%></span>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
	          <span><%=sdao.store_Select_Storeid((list.get(i).getStore_Id())).getStore_Name()%></span><br>
	          <a href="https://map.kakao.com/link/roadview/<%=sdao.store_Select_Storeid((list.get(i).getStore_Id())).getLatitude()%>,<%=sdao.store_Select_Storeid((list.get(i).getStore_Id())).getLongitude()%>"><%=sdao.store_Select_Storeid((list.get(i).getStore_Id())).getStore_Addr()%></a><br>
	          <span><%=sdao.store_Select_Storeid((list.get(i).getStore_Id())).getStore_Tel()%></span>
          </td>
        </tr>
 </table>
 ...
 <table class="type05">
  ...
  <%  
	...
	String menu_name = mdao.select_name(list.get(i).getMenu_Id());
	Integer price = list.get(i).getMenu_Price();
 	Long cnt = list.get(i).getMenu_Cnt();
	total += price*cnt;
	%>
	 <tr>
